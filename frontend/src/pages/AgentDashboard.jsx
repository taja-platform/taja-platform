...
// We'll use mock icons since we can't import lucide-react in a single file environment
const LayoutDashboard = (props) => <svg {...props} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z" /></svg>;
const User = (props) => <svg {...props} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>;
const Store = (props) => <svg {...props} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2h2a2 2 0 012 2v2M7 21h10a2 2 0 002-2v-6m-4 0v-2" /></svg>;
const MapPin = (props) => <svg {...props} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.828 0l-4.243-4.243a8 8 0 1111.314 0z"/><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg>;
const LogOut = (props) => <svg {...props} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 16l-4-4m0 0l4-4m-4 4h14m-5 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3v-5a3 3 0 013-3h5" /></svg>;
const MenuIcon = (props) => <svg {...props} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" /></svg>;
const XIcon = (props) => <svg {...props} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>;
...
const initialShops = [
    { id: 1, name: "QuickStop Groceries", address: "45 Elm Rd", latitude: 34.0522, longitude: -118.2437, state: "CA", date_created: "2024-05-10" },
    { id: 2, name: "The Corner Pharmacy", address: "800 Pine Ave", latitude: 34.045, longitude: -118.25, state: "CA", date_created: "2024-06-15" },
    { id: 3, name: "Tech Repair Hub", address: "99 Broadway", latitude: 34.06, longitude: -118.2, state: "CA", date_created: "2024-07-01" },
];
...
const ShopCard = ({ shop, onEdit }) => (
    <div className="bg-white p-5 rounded-xl shadow-lg border border-gray-100 transition-all duration-200 hover:shadow-xl hover:border-gray-200">
        ...
        <div className="grid grid-cols-2 gap-2 text-sm text-gray-700 mb-4">
            <div>
                <span className="font-medium text-gray-500">Latitude:</span> {shop.latitude}
            </div>
            <div>
                <span className="font-medium text-gray-500">Longitude:</span> {shop.longitude}
            </div>
            <div className="col-span-2">
                <span className="font-medium text-gray-500">Created:</span> {shop.date_created}
            </div>
        </div>
        <button
            onClick={() => onEdit(shop)}
...
const ShopFormModal = ({ shop, onClose, onSave }) => {
    ...
    const useCurrentLocation = () => {
        ...
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(
                ...
                (error) => {
                    mockToast.error("Could not get location. Ensure permissions are granted.");
    ...
    const handleSubmit = (e) => {
        ...
        setTimeout(() => {
            onSave(formData);
            mockToast.success(`Shop ${isEditing ? 'updated' : 'added'} successfully!`);
            setIsLoading(false);
        }, 1000);
    };
        ...
        <div className="fixed inset-0 bg-gray-900 bg-opacity-75 z-50 flex items-center justify-center p-4">
            <div className="bg-white w-full max-w-lg rounded-2xl p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    {/* Basic Info */}
                    <Input id="name" name="name" label="Shop Name" type="text" value={formData.name} onChange={handleChange} required />
                    <Input id="phone_number" name="phone_number" label="Phone Number" type="tel" value={formData.phone_number} onChange={handleChange} />
                    <Input id="address" name="address" label="Full Address" type="text" value={formData.address} onChange={handleChange} required />
                    <Input id="state" name="state" label="State/Region" type="text" value={formData.state} onChange={handleChange} required />

                    {/* Location Pinning Section */}
                    <div className="pt-3 border-t border-gray-100">
                        <h3 className="font-semibold text-gray-800 mb-3 flex items-center">
                            <MapPin className="w-5 h-5 mr-2 text-gray-900" /> Location Coordinates
                        </h3>

                        <div className="grid grid-cols-2 gap-4 mb-4">
                            <Input id="latitude" name="latitude" label="Latitude" type="number" step="0.000001" value={formData.latitude || ''} onChange={handleChange} readOnly={isLoading} required />
                            <Input id="longitude" name="longitude" label="Longitude" type="number" step="0.000001" value={formData.longitude || ''} onChange={handleChange} readOnly={isLoading} required />
                        </div>
                        ...
                        <div className="space-y-2">
                            ...
                            <div
                                ...
                                onClick={() => handleLocationPin(34.053 + (Math.random() * 0.01).toFixed(4), -118.243 - (Math.random() * 0.01).toFixed(4))}
                            >
        ...
        <div className="space-y-8">
            ...
            <div className="bg-white rounded-2xl p-6 shadow-xl border border-gray-100">
                ...
                <form onSubmit={handleProfileSubmit} className="space-y-6">
                    <Input id="agent_id" label="Agent ID" type="text" value={formData.agent_id} readOnly />
                    ...
                    <Input id="phone_number" name="phone_number" label="Phone Number" type="tel" value={formData.phone_number} onChange={handleChange} />
                    <Textarea id="address" name="address" label="Address" value={formData.address} onChange={handleChange} />
            ...
            <div className="bg-white rounded-2xl p-6 shadow-xl border border-gray-100">
                ...
                <form onSubmit={handlePasswordSubmit} className="space-y-6">
                    <Input id="current" name="current" label="Current Password" type="password" value={passwordData.current} onChange={handlePasswordChange} required />
                    <Input id="new" name="new" label="New Password" type="password" value={passwordData.new} onChange={handlePasswordChange} required />
                    <Input id="confirm" name="confirm" label="Confirm New Password" type="password" value={passwordData.confirm} onChange={handlePasswordChange} required />
...
const Input = ({ id, name, label, type = 'text', value, onChange, readOnly = false, required = false, step }) => {
    const [focused, setFocused] = useState(false);
    return (
        <div className="relative">
            <div className={`relative transition-all duration-200 ${focused || value ? 'transform -translate-y-2' : ''}`}>
                <label
                    htmlFor={id}
                ...
                <input
                    ...
                    step={step}
                    className={`w-full h-14 px-4 pt-6 pb-2 bg-gray-50 border transition-all duration-200 rounded-lg text-gray-900 placeholder-transparent ${readOnly ? 'bg-gray-100 cursor-not-allowed' : focused
        ...
        <div className="relative">
            <div className={`relative transition-all duration-200 ${focused || value ? 'transform -translate-y-2' : ''}`}>
                ...
                <textarea
                    ...
                    onFocus={() => setFocused(true)}
                    onBlur={() => setFocused(false)}
                    className={`w-full px-4 pt-6 pb-2 bg-gray-50 border transition-all duration-200 rounded-lg text-gray-900 placeholder-transparent ${focused
                        ? 'border-gray-900 bg-white shadow-sm'
                        : 'border-gray-300 hover:border-gray-400'
                        } focus:outline-none focus:ring-2 focus:ring-gray-900/10`}
                />
...
const AgentDashboard = () => {
    ...
    const shopsTodayCount = shops.filter(shop => shop.date_created === new Date().toISOString().split('T')[0]).length;

    const handleLogout = () => {
        // Mock logout logic
        console.log("Logged out");
        mockToast.info("Successfully logged out.");
    ...
    const handleSaveShop = (shopData) => {
        if (shopData.id) {
            ...
            setShops(prev => prev.map(s => s.id === shopData.id ? shopData : s));
        } else {
            // Add new shop
             // Safely find the next ID
            const maxId = shops.reduce((max, shop) => (shop.id > max ? shop.id : max), 0);
            const newId = maxId + 1;
            const newShop = {
                ...shopData,
                id: newId,
        ...
        <div className="space-y-6">
            ...
            {shops.length === 0 && (
                <div className="text-center py-10 bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200">
                    <p className="text-gray-500 font-medium">No shops added yet. Start by adding your first shop!</p>
                </div>
            )}
        </div>
    );
    ...
    return (
        <div className="min-h-screen bg-gray-100 font-sans flex">
                    ...
                    <div className="pt-4 border-t border-gray-700">
                        <div className="flex items-center space-x-3 p-3 text-white">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-gray-400">{profile.agent_id}</p>
                            </div>
                        </div>
                        <button
            ...
            {/* Main Content Area */}
            <main className={`flex-grow p-4 md:p-8 pt-20 md:pt-8 transition-all duration-300`}>
